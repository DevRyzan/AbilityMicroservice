...
public class DeleteAbilityComboHandlerTest
{
    ...
    public DeleteAbilityComboHandlerTest()
    {
    ...
    public async Task AbilityAttackStat_DeleteHandler_ValidRequest_ReturnsResponse(string id)
    {
        ...
        var handler = new DeleteAbilityComboCommandHandler(_abilityComboServiceMock.Object, businessRuleMock.Object, _mapperMock.Object);

        var request = new DeleteAbilityComboCommandRequest
        {
            DeleteAbilityComboDto = new DeleteAbilityComboDto
            {
                Id = id
            }
        };
        ...
        var expectedResponse = new DeleteAbilityComboCommandResponse
        {
            Id = id,
        ...
        _mapperMock.Setup(m => m.Map<AbilityCombo>(request.DeleteAbilityComboDto))
                   .Returns(abilityCombo);

        _abilityComboServiceMock.Setup(m => m.GetById(id))
                                        .ReturnsAsync(abilityCombo);

        _abilityComboServiceMock.Setup(m => m.Delete(abilityCombo))
                                        .ReturnsAsync(abilityCombo);

        _mapperMock.Setup(m => m.Map<DeleteAbilityComboCommandResponse>(abilityCombo))
        ...
        _abilityComboServiceMock.Verify(m => m.Delete(abilityCombo), Times.Once);
    }
